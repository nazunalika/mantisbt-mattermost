...
class MattermostPlugin extends MantisPlugin {
    ...
    function bugnote_add_edit($event, $bug_id, $bugnote_id, $files = null) {
        ...
        $this->notify($msg, $this->get_webhook($project), $this->get_channel($project), $this->get_text_attachment($this->bbcode_to_mattermost($note)));
    }
    ...
    function format_text($text) {
        return strip_tags(
            str_replace(
                array('&', '<', '>'),
                array('&amp;', '&lt;', '&gt;'),
                $this->bbcode_to_mattermost($text)
            )
        );
    }
    ...
    function notify($msg, $webhook, $channel, $attachment = FALSE) {
        ...
        if (empty($bot_icon)) {
            $payload['icon_url'] = 'https://i.imgur.com/jy9bjd0.png';
    ...
    function bbcode_to_mattermost($bbtext){
        $bbtags = array(
            '[b]' => '*','[/b]' => '* ',
            '[i]' => '_','[/i]' => '_ ',
            '[u]' => '_','[/u]' => '_ ',
